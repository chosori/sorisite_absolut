...
//   var bestitemBox = $('#newsBox');
//   var bestList = $('#newsList01');

//   bestList.children().clone(true).appendTo(bestList);
//   var newListFront = $('.news_tie01');
//   var newListBack = $('.news_tie02');
...
//   var bestLbtn = $('#news_btn').children('button').first();
//   var bestRbtn = $('#news_btn').children('button').last();
...
$.getJSON(jsonUrl,  function(data){ // console.log(data);
  var i          = 0, dataLength = data.length;
  for(; i<dataLength; i++){
    listBox.append('<li><a href="https://www.absolut.com/en/news/articles/a-night-for-change/" target="_blank"></a></li>');
    listBox.children('li').eq(i).find('a').append($('<img />',{alt:data[i].thum, src:thumDir + data[i].file }));
  }
...
// 모달창에서만 키포커스 움직이기
  ...
  //1. 모달 리스트내용을 일부만 보이게 만들자!
listWrap.css({height:'140px', overflow:'hidden', position:'relative'});
...
// 모달창에서만 키포커스 움직이기
  ...
  //1. 모달 리스트내용을 일부만 보이게 만들자!
listWrap.css({height:'160px', overflow:'hidden', position:'relative'});
listBox.css({position:'absolute', left:'50%', marginLeft:'-380px', top:'0'});
...
listViewBtn.css({position:'absolute', top:'50%', left:'0', marginTop:'-80px', /*marginLeft:'-100px',*/ zIndex:'100',
                      width:'100%', height:0});
listViewBtn.find('button').css({width:'50px', height:'100px', fontSize:'3rem'});
listLBtn.css({float:'left', marginLeft:'20px'});
listRBtn.css({float:'right',  marginRight:'20px'});
...
//1. 모달 리스트내용을 일부만 보이게 만들자!
listWrap.css({height:'130px', overflow:'hidden', position:'relative'});
...
listViewBtn.css({position:'absolute', top:'50%', left:'0', marginTop:'-60px', /*marginLeft:'-100px',*/ zIndex:'100',
                      width:'100%', height:0});
listViewBtn.find('button').css({width:'50px', height:'100px', fontSize:'3rem'});
...
//1. 모달 리스트내용을 일부만 보이게 만들자!
listWrap.css({height:'170px', overflow:'hidden', position:'relative'});
listBox.css({position:'absolute', left:'50%', marginLeft:'-610px', top:'0'});
...
listViewBtn.css({position:'absolute', top:'50%', left:'0', marginTop:'-50px', /*marginLeft:'-100px',*/ zIndex:'100',
                      width:'100%', height:0});
listViewBtn.find('button').css({width:'50px', height:'100px', fontSize:'3rem'});
listLBtn.css({float:'left', marginLeft:'50px'});
listRBtn.css({float:'right',  marginRight:'50px'});
